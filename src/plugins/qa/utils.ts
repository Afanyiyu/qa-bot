...
declare module 'koishi' {
  interface App {
    qaHistory: Record<number, Dialogue>
  }
...
export namespace Dialogue {
  ...
  export function get<K extends Field>(
    ctx: Context,
    ids: number[],
    fields?: K[]
  ): Promise<Pick<Dialogue, K>[]>
  export async function get(
    ctx: Context,
    test: DialogueTest | number[],
    fields?: Field[]
  ): Promise<Dialogue[] | Pick<Dialogue, any>[]> {
  ...
  export function addHistory(
    ...
    if (revert) return delete argv.app.qaHistory[dialogue.id]
    argv.app.qaHistory[dialogue.id] = dialogue
    const time = Date.now()
    ...
    setTimeout(() => {
      if (argv.app.qaHistory[dialogue.id]?._timestamp === time) {
        delete argv.app.qaHistory[dialogue.id]
      }
...
export function sendResult(
  ...
  if (updated.length) {
    output.push(
      create
        ? `修改了已存在的问答，编号为 ${updated.join(', ')}。`
        : `问答 ${updated.join(', ')} 已成功修改。`
    )
  }
  if (skipped.length) {
    output.push(
      create
        ? `问答已存在，编号为 ${target.join(', ')}，如要修改请尝试使用 ${
            config.prefix
          }${skipped.join(',')} 指令。`
        : `问答 ${skipped.join(', ')} 没有发生改动。`
    )
  }
  if (uneditable.length) {
    output.push(
      `问答 ${uneditable.join(', ')} 因权限过低无法${
        revert ? '回退' : remove ? '删除' : '修改'
      }。`
    )
  }
  if (unknown.length) {
    output.push(
      `${revert ? '最近无人修改过' : '没有搜索到'}编号为 ${unknown.join(
        ', '
      )} 的问答。`
    )
  }
...
export function prepareTargets(
  argv: Dialogue.Argv,
  dialogues = argv.dialogues
): Observed<Dialogue, void>[] {
