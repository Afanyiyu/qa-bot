...
export default function apply(ctx: Context, config: Dialogue.Config): void {
  ...
  ctx.on(
    'dialogue/permit',
    ({ session, target, options, authMap }, { writer, flag }) => {
  ...
  ctx.on('dialogue/before-send', async (state) => {
    ...
    if (
      dialogue.flag & Dialogue.Flag.substitute &&
      dialogue.writer &&
      session.user.id !== dialogue.writer
    ) {
