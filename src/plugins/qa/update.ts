...
export default function apply(ctx: Context): void {
  ctx
    ...
    .option('includeLast', '-l [count]  包含最近的修改数量', {
      type: isIntegerOrInterval,
    })
    .option('excludeLast', '-L [count]  排除最近的修改数量', {
      type: isIntegerOrInterval,
    })
  ...
  ctx.on('dialogue/execute', (argv) => {
    ...
    try {
      ...
      ctx.logger('qa').warn(err)
      return argv.session.send(
        `${revert ? '回退' : remove ? '删除' : '修改'}问答时出现问题。`
      )
    }
  })
  ...
  ctx.on(
    ...
    (argv) => {
      ...
      const now = Date.now(),
        includeTime = Time.parseTime(includeLast),
        excludeTime = Time.parseTime(excludeLast)
      const dialogues = Object.values(argv.app.qaHistory)
        .filter((dialogue) => {
        ...
        .sort((d1, d2) => d2._timestamp - d1._timestamp)
        .filter((_, index, temp) => {
  ...
  ctx.on('dialogue/detail-short', ({ _type, _timestamp }, output) => {
    if (_type) {
      output.unshift(
        `${_type}-${Time.formatTimeShort(Date.now() - _timestamp)}`
      )
    }
  })
  ...
  ctx.on(
    'dialogue/detail',
    ({ original, answer, flag, _type, _timestamp }, output) => {
...
async function revert(dialogues: Dialogue[], argv: Dialogue.Argv) {
  try {
    ...
    argv.app.logger('qa').warn(err)
    return argv.session.send('回退问答中出现问题。')
  }
}

export async function update(argv: Dialogue.Argv): Promise<string | void> {
  ...
  options.modify =
    !review && !search && (Object.keys(options).length || args.length)
  ...
  argv.updated = []
  argv.skipped = []
  const dialogues = (argv.dialogues =
    revert || review
      ? Object.values(pick(argv.app.qaHistory, target)).filter(Boolean)
  ...
  if (!options.modify) {
    if (argv.unknown.length) {
      await session.send(
        `${review ? '最近无人修改过' : '没有搜索到'}编号为 ${argv.unknown.join(
          ', '
        )} 的问答。`
      )
    }
    for (let index = 0; index < dialogues.length; index++) {
      const output = [
        `编号为 ${dialogues[index].id} 的${review ? '历史版本' : '问答信息'}：`,
      ]
...
export async function create(argv: Dialogue.Argv): Promise<string | void> {
  const {
    app,
    options,
  ...
  argv.updated = []
  argv.skipped = []
  argv.dialogues = await Dialogue.get(app, { question, answer })
  await app.serial('dialogue/before-detail', argv)
  const result = await app.serial('dialogue/before-modify', argv)
  if (typeof result === 'string') return result
